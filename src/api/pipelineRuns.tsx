...
export const fetchPipelineRun = async (pipelineRun: string) => {
    try {
        return await axios.get<PipelineRuns>(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/pipelineRuns/${pipelineRun}`).then((res) => res.data);
    } catch (error) {
        console.error('Error fetching pipelineRun:', error);
    }
};

export const fetchApprove = async (pipelineRun: string, worker: string, message: string) => {
    const req: reqApproveDeny = {
        worker, message
    }

    try {
        return await axios.post<ResApproveDeny>(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/approve/${pipelineRun}`, req).then((res) => res.data.result);
    } catch (error) {
...
export const fetchDeny = async (pipelineRun: string, worker: string, message: string) => {
    const req: reqApproveDeny = {
        worker, message
    }

    try {
        return await axios.post<ResApproveDeny>(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/deny/${pipelineRun}`, req).then((res) => res.data.result);
    } catch (error) {
